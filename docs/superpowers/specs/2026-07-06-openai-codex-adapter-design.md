...
OpenAI or Gemini coding-agent session has ever actually been ingested, so those provider buckets are
permanently zero.
...
entry and one more `PROVIDER_ADAPTERS` entry, with no dispatch logic changes.
...
- **Commits/PRs:** `function_call` events (`payload.name == "shell"`, the actual command in
  ...
  `GITHUB_REPO_RE`, the same pending-call-id-set pattern as `claude_code.py`, and the same "findall not
...
- Detecting/special-casing which actual backend model a Codex CLI session routed to (local Ollama,
  OpenRouter, etc.); it's always labeled `provider="openai"`.
