...
2. **AI-40** — flip `deletion_protection = false` to `true` on `google_sql_database_instance.main`
   ...
   metadata-only flag — `terraform apply` shows it as a pure in-place change, no replacement or
...
- **AI-39 (lifecycle block):** apply the `ignore_changes` addition via `terraform apply` (a
  Terraform-only metadata change — no corresponding GCP-side diff). After a real CI deploy has
...
- Pin tag = git commit SHA (not Cloud Build ID or timestamp) — directly traceable to the exact
  code that's running.
- Drop the `:latest` tag entirely rather than keeping it alongside the SHA tag — no floating
  reference left in Artifact Registry going forward.
