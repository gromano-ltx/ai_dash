...
- **Verified duplication:** real sessions on this machine log the same message `id` twice in a row
  ...
  accumulating tokens/text/tool calls — a `seen_ids: set[str]` mirroring `claude_code.py`'s
...
approach — Gemini has no single "assistant-only" timestamp column the way Claude Code's
...
plain string at `args.command` (unlike Codex's array-wrapped command — no `' '.join()` needed), and
...
initial `$set.messages[0]`) whose text doesn't start with `<` — covers both `<session_context>`
...
(path-aware, done in `parse_transcript(path)`, not the content parser — same separation of concerns
as `claude_code.py`'s subagent-path detection). For subagent sessions, take the first entry of the
...
`[cwd, first_user_text, label] + bash_commands` — `cwd` substituted for `git_branch` since branch
